...
from utils.plotting.plotting import plot_mc_curve, plot_pr_curve
...
def average_precision_per_class(tp,
                 conf,
                 pred_cls,
                 target_cls,
                 plot=False,
    ...
    for ci, c in enumerate(unique_classes):
        ...
        for j in range(tp.shape[1]):
            ap[ci, j], mpre, mrec = compute_average_precision(recall[:, j], precision[:, j])
            if plot and j == 0:
